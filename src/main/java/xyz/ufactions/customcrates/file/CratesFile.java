...
public class CratesFile {
	...
	public Crate createCrate(String identifier) {
		...
		try {
			...
			plugin.getLogger()
					.warning("Failed to create crate file. Please submit the following error when making a ticket.");
			e.printStackTrace();
		}
		...
		config.set(path + ".open-commands",
				Collections.singletonList("broadcast &e%player% &bis opening &e%crate%&b."));
		...
		config.set(path + ".lines", Arrays.asList(
				/* "{item_key}", */
				"{crate_display}", "&7(Right Click to Open)", "&7(Left Click to Preview)"));

		// Create Prizes

		ItemBuilder builder = new ItemBuilder(Material.DIAMOND).glow(true).name("&b&lDIAMONDS!!")
				.lore(Collections.singletonList("&aChance: &e%chance%"));
		Prize prize = new Prize(builder, 50, "Prizes.diamond",
				Arrays.asList("broadcast &e%player% &ahas won &e2x &b&lDIAMONDS!", "give %player% diamond 2"));
		savePrize(config, prize);

		builder = new ItemBuilder(Material.GOLD_INGOT).glow(false).name("&6&lGold")
				.lore(Collections.singletonList("&aChance: &e%chance%"));
		prize = new Prize(builder, 50, "Prizes.gold", Collections.singletonList("give %player% gold_ingot 1"));
		savePrize(config, prize);
		...
		try {
			...
			plugin.getLogger()
					.warning("Failed to save new crate file. Please submit the following error when making a ticket:");
			e.printStackTrace();
		}
	...
	public boolean createPrize(Crate crate, Prize prize) {
		...
		try {
			...
			plugin.getLogger().warning(
					"Failed to save configuration file. Please submit the following error when making a ticket:");
			e.printStackTrace();
		}
		return true;
	}
	...
	private void savePrize(FileConfiguration config, Prize prize) {
		...
		try {
			SkullMeta skullMeta = (SkullMeta) prize.getDisplayItem().getItemMeta();
			if (skullMeta.getDisplayName().toString().equals("John Doe")) {
				config.set(prize.getConfigurationSection() + ".display.owner", skullMeta.getLocalizedName());
			} else {
				config.set(prize.getConfigurationSection() + ".display.owner",
		...
		config.set(prize.getConfigurationSection() + ".display.name",
				prize.getDisplayItem().getItemMeta().getDisplayName());
	...
	public Crate getCrateByFile(File file) {
		...
		// Load Sounds
		Sound openingSound = parseSound(configuration.getString("Crate.open sound", ""), identifier, "CHEST_OPEN",
				"BLOCK_CHEST_OPEN");
		Sound spinSound = parseSound(configuration.getString("Crate.spin sound", ""), identifier, "NOTE_PLING",
				"BLOCK_NOTE_PLING");
		Sound winSound = parseSound(configuration.getString("Crate.win sound", ""), identifier, "LEVEL_UP",
		...
		if (keyBuilder == null)
			keyBuilder = new ItemBuilder(Material.AIR);
		...
		try {
			...
			plugin.getLogger().warning("Could not load spin type for \"" + identifier + "\". '"
					+ configuration.getString("Crate.spin", "NaN") + "'");
			if (plugin.debugging())
				e.printStackTrace();
		}
		...
		if (!holographicLines.isEmpty()) { // Redundant check most of the times but if holograms aren't configured we
											// aren't going to load the rest
		...
		// Construction
		CrateSettings settings = new CrateSettings(identifier, display, openCommands, block, spinType, spinTime,
				openingSound, spinSound, winSound, keyBuilder, pouchBuilder, prizes, holographicItemMap,
				holographicLines);
	...
	private ItemBuilder itemFromConfiguration(FileConfiguration config, String path) {
		...
		if (material == null)
			materialFromConfiguration(config, path + ".material");
		if (material == null)
		...
		String player = config.getString(path + ".owner");
		if (player != null) {
		...
		for (String serializedEnchantment : config.getStringList(path + ".enchantments")) {
			if (!serializedEnchantment.contains(":")) {
				plugin.getLogger().warning(
						"Enchantment format incorrect '" + path + "'. Must be in format {ENCHANTMENT NAME}:{LEVEL}");
				continue;
			}
		...
		try {
			...
			if (sound == null)
				if (plugin.getConfigurationFile().debugging())
					plugin.getLogger().info("Could not load closing sound for crate: \"" + identifier
							+ "\". Defaulting to: " + sound.name() + ".");
			return sound;
		}
	}
