...
public class Crate {
    ...
    public Crate(CrateSettings settings) {
        ...
        this.previewInventory = Bukkit.createInventory(null, UtilMath.roundInventorySize(settings.getPrizes().size()),
                settings.getDisplay());

        int delta = 0;
        for (RandomizableList<Prize>.Entry entry : settings.getPrizes()) {
            previewInventory.setItem(delta++, entry.getObject().getItemBuilder().build());
        }
    }
